...
import kotlite.aux.sort.NullsOrder
...
class MyClassRepositoryTest {
    ...
    fun rollback() {
        ...
        val items = db.transaction { myClassRepository.findAll() }
    ...
    fun `fail on conflict`() {
        ...
        val msg = expect<Exception> {
            db.transaction { myClassRepository.saveOrFail(item) }
        }.message

        assert("A PRIMARY KEY constraint failed" in msg!!)
    }
    ...
    fun `single result query method throws if there are more than one result`() {
        ...
        expect<IllegalStateException> {
            db.transaction { myClassRepository.findSingleBySpecProc("bionic13") }
        }
    }
    ...
    fun `not null method throws if there is no result`() {
        expect<NoSuchElementException> {
            db.transaction { myClassRepository.findSingleById(item.id) }
        }
    }
    ...
    fun `multiple parameters combined with AND`() {
        ...
        fun `find by id and version`(id: String, version: Int) =
            db.transaction { myClassRepository.findByIdAndVersion(id, version) }
    ...
    fun order() {
        ...
        all(
            ...
            },
            {
                ...
                assert(actual == listOf(null, "item12", "item13")) { "order by name" }
            },
            {
